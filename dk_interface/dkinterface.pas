...
var
  ...
  connected,NeedConnect,Connecting,goDisconnect:Boolean;
...
pcount:=0;
 ...
 Form1.Caption:='DK Report Interface v.'+FileVersion(Paramstr(0))+' [Динские Колбасы]';
...
begin
 ...
 try
  ...
  if (Pos('08:00:',Timetostr(time))>0) or (Pos('09:00:',Timetostr(time))>0)
   or (Pos('08:01:',Timetostr(time))>0) or (Pos('09:01:',Timetostr(time))>0)
   then AutoZapros;
  Timer2.Enabled:=True;
 end;
...
begin
 try
  ...
  ShowMessage('Архив с остатками будет помещен в C:\Инвентаризация\');
  N30.Enabled:=False;
  except
...
begin
 ...
 Form16.ComboBox1.Items.Append('ВводИзлишковТовара');
