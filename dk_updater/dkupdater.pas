        ...
        try
         ...
         try
          ...
          begin
           ...
           if FileExists(GetSpecialPath(CSIDL_COMMON_STARTUP)+'\dkrupdater.lnk') then
            DeleteFile(GetSpecialPath(CSIDL_COMMON_STARTUP)+'\dkrupdater.lnk');
           if FileExists(GetSpecialPath(CSIDL_COMMON_STARTUP)+'\dkengine.lnk') then
            DeleteFile(GetSpecialPath(CSIDL_COMMON_STARTUP)+'\dkengine.lnk');
           if FileExists(GetSpecialPath(CSIDL_COMMON_STARTUP)+'\dk_interface.lnk') then
            DeleteFile(GetSpecialPath(CSIDL_COMMON_STARTUP)+'\dk_interface.lnk');

              try
              DeleteFile('dk_interface.exe_');
              DeleteFile('dkengine.exe_');
              DeleteFile('dkrupdater.exe_');
              RenameFile('dk_interface.exe','dk_interface.exe_');
              RenameFile('dkengine.exe','dkengine.exe_');
              RenameFile('dkrupdater.exe','dkrupdater.exe_');
             except
               end;
           ...
           if CopyFile(PChar(extractfilepath(paramstr(0))+'\updscript.lnk'),PChar(GetSpecialPath(CSIDL_COMMON_STARTUP)+'\updscript.lnk'),false) then
              try
...
begin
 ...
 for i:=0 to process.Count-1 do
  if process[i]='dkrupdater.exe' then pcount:=pcount+1;
...
if not DirectoryExists('update_temp') then CreateDir('update_temp');
 ...
 try
  ...
  dir:=ini.ReadString('Updater','Dir','update_v2');
  ...
  sdir:=ini.ReadString('FTP','Dir','update_v2');
  Ini.Free;
    except
     end;
 ...
 if not FileExists(GetSpecialPath(CSIDL_COMMON_STARTUP)+'dkrupdater.lnk') then
    CopyFile(PChar(extractfilepath(paramstr(0))+'\dkrupdater.lnk'),PChar(GetSpecialPath(CSIDL_COMMON_STARTUP)+'\dkrupdater.lnk'),false);

 if not FileExists(extractfilepath(paramstr(0))+'\updscript.lnk') then
  begin
   ShowMessage('Создайте ярлык для updscript.exe с именем updscript.lnk в папке с программой!');
   Application.Terminate;
    end;
