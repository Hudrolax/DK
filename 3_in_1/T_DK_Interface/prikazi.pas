...
begin
 ...
 FileList.Clear;
 ScanDir(GetSpecialPath(CSIDL_COMMON_DESKTOPDIRECTORY )+'\Приказы_Одежда\','t*.txt',FileList);
 ...
 for i:=0 to FileList.Count-1 do
  begin
   ...
   AssignFile(f,GetSpecialPath(CSIDL_COMMON_DESKTOPDIRECTORY )+'\Приказы_Одежда\'+Filelist[i]);
   Reset(f);
   while not Eof(f) do
    begin
     Readln(f,s);
...
begin
   if FileExists(GetSpecialPath(CSIDL_COMMON_DESKTOPDIRECTORY )+'\Приказы_Одежда\'+sg1.Cells[2,PrikaziArowS]+'.txt') then
  begin
   form11.Memo1.Lines.LoadFromFile(GetSpecialPath(CSIDL_COMMON_DESKTOPDIRECTORY )+'\Приказы_Одежда\'+sg1.Cells[2,PrikaziArowS]+'.txt');
