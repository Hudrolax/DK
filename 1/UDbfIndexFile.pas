...
//------------------------------------------------------------------------------
  ...
  protected
    ...
    function  CheckAcceptable(forward:boolean):boolean;
...
//    _MdxHdr : rMdxHdr;
  ...
  public
    ...
    procedure Clear;
    procedure Init;
    procedure SetFieldInfo(fieldType:char; pfieldStart,pfieldLen:integer; FieldDesc:string);
    ...
    procedure SetPosLen(pPos,pLen:integer);
  end;
...
procedure TIndexFile.SetFieldInfo(fieldType:char; pfieldStart,pfieldLen:integer; FieldDesc:string);
begin
  PNdxHdr(Header).keytype:=fieldType;
  PNdxHdr(Header).keylen:=pfieldLen;
  PNdxHdr(Header).keyreclen:=pfieldLen+8;
  ...
  PNdxHdr(Header).nbkey:=(512-8) div (pfieldLen+8);
  StrLCopy(PNdxHdr(Header).KeyDesc,PChar(UpperCase(fieldDesc)),255);
  WriteHeader;
  RecordCount:=0;
  _Root.SetPageNo(PNdxHdr(Header).startpage);
  _FieldPos := pfieldstart;
  _FieldLen := pfieldlen;
end;
...
begin
  ...
  if EntryNo>0 then begin
    EntryNo:=EntryNo-1;
    if CheckAcceptable(false) then begin
      if (_LowerLevel<>nil) then begin
...
begin
  ...
  if (EntryNo<LastEntryNo) then begin
    EntryNo:=EntryNo+1;
    if CheckAcceptable(true) then begin
      if (_LowerLevel<>nil) then begin
...
function  TIndexPage.CheckAcceptable(forward:boolean):boolean;
var
  acceptable:boolean;
  pfirst:pchar;
  plast:pchar;
begin
  while (EntryNo>=0) and (EntryNo<=LastEntryNo) do begin
    if EntryNo>0 then pfirst:=PCHAR(integer(@_Entry.CKey) - PNdxHdr(_IndexFile.Header).keyreclen)
    else pfirst:=nil;
    if EntryNo<=LastEntryNo then plast:=_Entry.CKey
    else plast:=nil;

    _IndexFile.OnBracketStr(
        pfirst,
        plast,
        EntryNo=0,
        EntryNo=LastEntryNo,
    ...
    if forward then EntryNo:=EntryNo+1
    else EntryNo:=EntryNo-1
  end;
  result:=false;
end;
...
procedure TIndexFile.SetPosLen(pPos,pLen:integer);
begin
  _FieldPos :=pPos;
  _FieldLen :=pLen;
end;
