...
//====================================================================
  ...
  private
    ...
    procedure _OnBracketStr(First, Last: PChar;NoFirst,NoLast:boolean;var Accept: Boolean);
    procedure _OnBracketNum(First, Last: double;NoFirst,NoLast:boolean;var Accept: Boolean);
    procedure _SetBracketLow(Value:string);
    procedure _SetBracketHigh(Value:string);
  public
...
{$endif}
  ...
  public
    ...
    destructor Destroy; override;
  published
    property BracketLow:string read _BracketLow write _SetBracketLow;
    property BracketHigh:string read _BracketHigh write _SetBracketHigh;
//    property MasterSource: TDataset read _MasterSource write _MasterSource;
//    property MasterField: string read _MasterField write _MasterField;
...
var
  ...
  s:string;
begin
  if (_DbfFile.RecordCount<1) or (_Cursor=nil) then  begin
    Result := grEOF;
    exit;
  end;
  prec:=pDBFRecord(Buffer);
...
//TFieldAttributes = set of TFieldAttribute;
          FieldDefs[Il].Attributes:=[db.faReadOnly];
    {$endif}
  end;
end;
...
begin
  ...
  try
    ...
    _DbfFile.FinishCreate(aDbfFieldDefs,'.dbt',512);
  finally
    aDbfFieldDefs.Free;
  end;
end;
...
begin
  ...
  try
    ...
    if DbfFieldDefs=nil then begin
      ...
      _DbfFile.FinishCreate(DbfFieldDefs,'.dbt',512);
    end;
...
begin
  ...
  if DbfIndex<>nil then begin
    ...
    TIndexCursor(_Cursor).OnBracketStr:=_OnBracketStr;
    TIndexCursor(_Cursor).OnBracketNum:=_OnBracketNum;
  end else begin
    FreeAndNil(_Cursor);
...
procedure TDbf._OnBracketStr(First, Last: PChar;NoFirst,NoLast:boolean;
  var Accept: Boolean);
begin
  if (_Cursor = nil)
    or ((length(_BracketLow)=0) and (length(_BracketLow)=0)) then begin
    Accept:=true;
    exit;
  end else if _Cursor is TIndexCursor then begin
    Accept:=(Last>=_BracketLow) and
      (First<=_BracketHigh);
  end;
end;
procedure TDbf._OnBracketNum(First, Last: double;NoFirst,NoLast:boolean;
          var Accept: Boolean);
begin
  if (_Cursor = nil)
    or (length(_BracketLow)=0) and (length(_BracketLow)=0) then begin
    ...
    if length(_BracketLow)>0 then Accept:=(Last>=_fBracketLow)
    else Accept:=true;
    if (length(_BracketHigh)>0) and Accept then Accept:=(First<=_fBracketHigh);
  end;
end;
procedure TDbf._SetBracketLow(Value:string);
begin
  _BracketLow:=Value;
  try
    _fBracketLow:=StrToFloat(Value);
  except
  end;
  Refresh();
end;

procedure TDbf._SetBracketHigh(Value:string);
begin
  _BracketHigh:=Value;
  try
    _fBracketHigh:=StrToFloat(Value);
  except
  end;
  Refresh();
end;
