...
begin
  if not fileExists(_FileName) then begin
    if AutoCreate or (Mode = pfCreate) then fileopenmode:=(fmCreate or fmShareDenyWrite)
    else raise EPagedFile.CreateFmt(STRING_FILE_NOT_FOUND,[_FileName]);
  end else begin
    if ReadOnly  then fileopenmode := fmOpenRead or fmShareDenyNone
    else fileopenmode := fmOpenReadWrite + fmShareDenyWrite;
  end;
