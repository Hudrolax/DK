...
//    PDbfHdr(Header).Language:='X';
  ...
  if _HasBlob and (_dbtFile=nil) then begin
    lMemoFileName:=ChangeFileExt(_FileName,MemoExt); // usually '.dbt' excepy for packtable
...
begin
  ...
  try
    ...
    if _dbtFile<>nil then
      pbfFile.FinishCreate(_FieldList,'.pbt',_dbtFile.RecordSize)
    else
      pbfFile.FinishCreate(_FieldList,'.pbt',512);
...
var
  ...
  IntValue:integer;
begin
  fillchar(p^,RecordSize,' ');
  for i := 0 to _FieldList.Count-1 do begin
    MyFieldDef:=_FieldList.Items[i];
    if (MyFieldDef.NativeFieldType in ['I', 'O', '@']) then begin
       fillchar(pchar(p+MyFieldDef.Offset)^,MyFieldDef.Size,0);
    end else if (MyFieldDef.NativeFieldType = '+') then begin
...
//  lFileName:=IndexFileName;
  ...
  if lIndexFile=nil then begin
    ...
    if mode=pfOpen then begin
      ...
      //lIndexFile.SetFieldInfo(lFieldInfo.Offset,lFieldInfo.Size,'')
      ...
      lIndexFile.SetPosLen(lFieldInfo.Offset,lFieldInfo.Size);
    end else begin
      ...
      lIndexFile.SetFieldInfo('C',lFieldInfo.Offset,lFieldInfo.Size,IndexField);
    end;
...
begin
  ...
  if (DbfFile._cntUse<=0) then begin
    idx:=_DbfFiles.IndexOf(DbfFile);
    if idx>=0 then _DbfFiles.Delete(idx);
    FreeAndNil(DbfFile);
  end;
  DbfFile:=nil;
end;
