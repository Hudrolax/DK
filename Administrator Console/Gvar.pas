...
interface
uses SysUtils, Types, Windows, TLHELP32, Classes, StdCtrls, DateUtils;
...
function datefromftp(d:string):string;
...
begin
 ...
 for i:=1 to Length(d)-1 do
  ...
  if k=2 then
    begin
      DFFTP.dtime:=Copy(d,Length(d)-4,5);
      Break;
      end;
  end;
 for i:=1 to Length(d)-1 do
  if d[i]=' ' then
   begin
    DFFTP.day:=d[i+1]+d[i+2];
    Break;
   end;
 if Copy(d,1,3)='Jan' then
  DFFTP.mounth:='Января';
 if Copy(d,1,3)='Feb' then
  DFFTP.mounth:='Февраля';
 if Copy(d,1,3)='Mar' then
  DFFTP.mounth:='Марта';
 if Copy(d,1,3)='Apr' then
  DFFTP.mounth:='Апреля';
 if Copy(d,1,3)='May' then
  DFFTP.mounth:='Мая';
 if Copy(d,1,3)='Jun' then
  DFFTP.mounth:='Июня';
 if Copy(d,1,3)='Jul' then
  DFFTP.mounth:='Июля';
 if Copy(d,1,3)='Aug' then
  DFFTP.mounth:='Августа';
 if Copy(d,1,3)='Sep' then
  DFFTP.mounth:='Сентября';
 if Copy(d,1,3)='Oct' then
  DFFTP.mounth:='Октября';
 if Copy(d,1,3)='Nov' then
  DFFTP.mounth:='Ноября';
 if Copy(d,1,3)='Dec' then
  DFFTP.mounth:='Декабря';

  DFFTP.year:=Copy(DateToStr(date),Length(DateToStr(date))-3,4);

  Result:=DFFTP.day+' '+DFFTP.mounth+' '+DFFTP.year+' '+DFFTP.dtime;
end;
