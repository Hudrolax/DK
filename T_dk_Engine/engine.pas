     ...
     try
      ...
      // Создаем флаг загрузки для POS
        begin
         ...
         Writeln(log,getdatet+' Файл '+Form1.DBFPTP.GetFieldData(1)+' распакован в C:\T_pos\pos0'+s);
         Break;
        end;
     except
     Writeln(log,getdatet+' Файл '+Form1.DBFPTP.GetFieldData(1)+' не распакован в C:\T_pos\pos0'+s);
 ...
 try
  ...
  for i:=posn to posn do
   if FileExists('c:\T_pos\pos0'+inttostr(i)+'\pos.rep') then begin
     ...
     // Если нашли файл, делаем пометку в логе
     Writeln(log,getdatet+' Файл pos.rep был найден в папке c:\T_pos\pos0'+inttostr(i)+'\');
     Writeln(log,getdatet+' Жду 5 секунд, вдруг файл не записался до конца.');
     Sleep(5000);
     h:=FileOpen('c:\T_pos\pos0'+inttostr(i)+'\pos.rep',fmShareExclusive);   //считываем дискриптор
       FileClose(h);  // закрываем файл
     if h<=0 then    // если больше нуля - знач все ОК
          begin
           Writeln(log,getdatet+' Файл '+'c:\T_pos\pos0'+inttostr(i)+'\pos.rep'+' занят какой-то херней, пропускаем.');
           Continue;
          end;
...
ls.Clear;
 try // Получаем листинг с ФТП
  writeln(log,Getdatet+'(getinv) Пробую получить список флагов из папки '+dir);
  ...
  try
  writeln(log,Getdatet+'(getinv) Не могу получить список файлов из папки '+dir);
   except
     end;
  ls.Free;
  flist.Free;
 ...
 //Фильтруем, оставляя флаги загрузки.
   ...
   writeln(log,Copy(ls[i],AnsiPos('.gin',ls[i])-8,12));
     ...
     try
    writeln(log,Getdatet+'(getinv) Не могу отфильтровать список файлов из папки '+dir);
   except
     end;
    ls.Free;
    flist.Free;
 ...
 if flist.Count<>0 then
  if not DirectoryExists('c:\Инвентаризация_Шмоточка') then
    CreateDir('c:\Инвентаризация_Шмоточка');
 ...
 if flist.Count<>0 then
  ...
  begin
   ...
   if CopyFile(PChar('temp\'+Copy(flist[i],1,8)+'.zip'),Pchar('c:\Инвентаризация_Шмоточка\'+Copy(flist[i],1,8)+'.zip'),false) then
    begin
...
ls.Clear;
 ...
 //Фильтруем, оставляя флаги загрузки.
   ...
   writeln(log,Copy(ls[i],AnsiPos('.gid',ls[i])-8,12));
     ...
     try
    writeln(log,Getdatet+'(getdoc) Не могу отфильтровать список файлов из папки '+dir);
   except
     end;
    ls.Free;
    flist.Free;
 ...
 if flist.Count<>0 then
  ...
  begin
   ...
   Form1.ftp.get(Copy(flist[i],1,8)+'.zip','temp\'+Copy(flist[i],1,8)+'.zip',true,false);
    ...
    if not DirectoryExists('C:\Инвентаризация_Шмоточка\'+Copy(flist[i],1,8)) then
      CreateDir('C:\Инвентаризация_Шмоточка\'+Copy(flist[i],1,8));
     with form1.zp1 do
      try
      FileName:='temp\'+Copy(flist[i],1,8)+'.zip';
      OpenArchive(fmOpenRead);
      BaseDir:='C:\Инвентаризация_Шмоточка\'+Copy(flist[i],1,8)+'\';
      ExtractFiles('*.*');
      CloseArchive;
      except
       ls.Free;
...
runload:=False;
 ...
 try
  ...
  if runload then
   try
   Writeln(log,getdatet+' Пробую закачать скрипт T_runscript.exe');
...
begin
  try
    ...
    try
      Writeln(log,getdatet+' PutToFTP: Не могу открыть БД upload.dbf.');
      Form1.DBF1.Close;
      except
        end;
     Exit;
...
ls.Clear;
 ...
 if flist.Count<>0 then
  ...
  // Пробуем распаковать приказы
      ...
      if CopyFile(PChar('temp\'+Copy(flist[i],1,8)+'.txt'),PChar(GetSpecialPath(CSIDL_COMMON_DESKTOPDIRECTORY )+'\Приказы_Одежда\'+Copy(flist[i],1,8)+'.txt'),False) then
        begin
          DeleteFile('temp\'+Copy(flist[i],1,8)+'.txt');
          DeleteFile('temp\demo.spr');
          ShellExecute(0,'Open',pchar(GetSpecialPath(CSIDL_COMMON_DESKTOPDIRECTORY )+'\Приказы_Одежда\'+Copy(flist[i],1,8)+'.txt'),nil,nil, 1);
          end;
...
//   end;
   ...
   if not DirectoryExists(GetSpecialPath(CSIDL_COMMON_DESKTOPDIRECTORY )+'\Приказы_Одежда') then
      CreateDir(GetSpecialPath(CSIDL_COMMON_DESKTOPDIRECTORY )+'\Приказы_Одежда');
  try
 ...
 try //пробуем писать лог
  ...
  AssignFile(log,'common.log');
  Append(log);
  writeln(log,Getdatet+' DK Report Engine запущен на '+GetComputerNName+' [ Шмоточка ]');
  except
  end;
  //биндим хоткей при запуске
  try
if not RegisterHotkey(Handle, 1, MOD_ALT, VK_F2) then
  ShowMessage('Клавиши Alt+F2 уже заняты.');
  except
    ShowMessage('Не могу зарегистрировать клавиши Alt+F2');
    writeln(log,Getdatet+' Не могу зарегистрировать клавиши Alt+F2');
    end;
 ...
 try
  ...
  if not FileExists('base\upload.dbf') then begin
   ...
   dbf1.CreateTable;
   DBF1.close;
   writeln(log,Getdatet+' Файл upload.dbf создан.');
  end;
  except
    writeln(log,Getdatet+' Не могу создать файл upload.dbf.');
    end;
...
//    writeln(log,Getdatet+' Не могу создать файл download.dbf.');
 ...
 try
  ...
  Form1.Caption:='DK Report Engine v.'+FileVersion(Paramstr(0))+' [ Шмоточка ]';
end;
...
//Убиваем хотей при закрытии.
 ...
 try
   writeln(log,Getdatet+' DK Report Engine закрыт на '+GetComputerNName+' [ Шмоточка ]');
   closefile(log);
 except
 end;
end;
...
if start then begin
 ...
 writeln(log,Getdatet+' DK Report Engine ВЫКЛЮЧЕН на '+GetComputerNName+' [ Шмоточка ]');
 ...
 writeln(log,Getdatet+' DK Report Engine ВКЛЮЧЕН на '+GetComputerNName+' [ Шмоточка ]');
